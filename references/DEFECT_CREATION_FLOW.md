...
2. Create `plato-workspace/tickets/<ticket-number>/` if it doesn't exist, and
   copy `skills/plato/templates/ticket/status.default.defect.json` into it as `status.json`.
...
4. Run `python skills/plato/scripts/generate_command.py <ticket-number> fixer TODO ""` —
