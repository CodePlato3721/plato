# Simple Feature Continue Flow

Reached when `status.json`'s `type` field is `simple_feature`.
...
## Step 2 — Find the active role

Run `python .plato/scripts/read_status/cli.py <ticket-number>`.
It prints four lines:

```
role: <designer|coder|none>
task-id: (always empty for simple feature tickets)
...
Run `python .plato/scripts/gen_cmd/cli.py <ticket-number> <role> <status> <session-id>`
