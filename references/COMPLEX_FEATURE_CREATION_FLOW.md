# Complex Feature Creation Flow

Reached when the ticket type (already determined by the entry point) is `complex_feature`.
...
3. Create `plato-workspace/tickets/<ticket-number>/` if it doesn't exist, and
   copy `templates/ticket/status.default.complex_feature.json` into it as `status.json`.
