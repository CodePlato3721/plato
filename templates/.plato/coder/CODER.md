...
**This ticket's `type` (read from status.json in Startup Rules) is the ONLY thing that decides whether you're in `simple_feature` or `complex_feature` mode below. Never infer the mode from whether a `task-id` happens to be present in the prompt, or from any other signal.**
...
- **task-id**: Read from `<task-id>` in the prompt. Only included in the prompt for `complex_feature` tickets — irrelevant either way, since **type** (not the prompt's shape) is what decides the mode.
...
- **type**: This ticket's `type` field in **status.json** — `simple_feature` or `complex_feature`
- **tasks.json**: All task statuses, path: `plato-workspace/tickets/<ticket-number>/tasks.json` — only exists for `complex_feature` tickets (the planner writes it; `simple_feature` tickets never run a planner)
...
1. Read **ticket-number** and **session-id** from the prompt (and **task-id**, if included).
2. Read **status.json** to get the ticket's status and **type**.
3. If **type** is `complex_feature`, read **tasks.json** to get task information. (Skip this for `simple_feature` — the file doesn't exist.)

## Execution Rules

Work through the following steps in order. Where a step differs by **type**, both variants are given.
...
- If it contains multiple `#`-headed sections (one per project root, e.g.
  ...
  - Look for signals in **DESIGN** and, for `complex_feature`, this task's
    entry in **tasks.json** (e.g. which root's files the task touches).
...
- **complex_feature**: Run `python .plato/scripts/write_status/cli.py coder run <ticket-number> <task-id> <session-id>` (sets this task's status to `IN_PROGRESS` and records this session's session-id; the task must already be registered in `coder.tasks` by the planner — this command does not create tasks)
- **simple_feature**: Run `python .plato/scripts/write_status/cli.py coder run <ticket-number> <session-id>` (sets `coder.status` to `IN_PROGRESS` and records this session's session-id)

### Step 3: Do the Work

- **complex_feature**: Work according to the instructions in **tasks.json** and **DESIGN**.
- **simple_feature**: Implement the entire feature according to **DESIGN** — there is no task breakdown; this single pass covers the whole feature.

### Step 4: Generate CR

After work is complete, **do not commit or push** — write the CR content, following the format defined in **CR**, to disk at **.cr.md**'s path (this must be a real file, not just text in your reply). Read the file back to confirm it was actually written before moving on. Then run:
- **complex_feature**: `python .plato/scripts/write_status/cli.py coder wait <ticket-number> <task-id>`
- **simple_feature**: `python .plato/scripts/write_status/cli.py coder wait <ticket-number>`
...
- **approve**:
  ...
  3. Run the approve command for this ticket's **type**:
     - **complex_feature**: `python .plato/scripts/write_status/cli.py coder approve <ticket-number> <task-id>`
     - **simple_feature**: `python .plato/scripts/write_status/cli.py coder approve <ticket-number>`
  4. **complex_feature**: Re-read **tasks.json** and this ticket's `coder.tasks` in **status.json**. If every task's `status` is now `DONE` (this was the last remaining task), tell the user: "Done. **Use `/exit` to leave this session**, then run `/plato <ticket-number>` to finish this ticket." and stop here — do not generate a next command. Otherwise:
  ...
  5. **simple_feature**: This was the only coder step — approving it marks `coder.status` `DONE`, which completes the ticket. Tell the user: "Done. **Use `/exit` to leave this session**, then run `/plato <ticket-number>` to finish this ticket." and stop here — do not generate a next command.

- **reject**:
  1. Revert all code changes from this session
  2. Run the reject command for this ticket's **type**:
     - **complex_feature**: `python .plato/scripts/write_status/cli.py coder reject <ticket-number> <task-id>`
     - **simple_feature**: `python .plato/scripts/write_status/cli.py coder reject <ticket-number>`
  3. Tell the user: "Change rejected. **Use `/exit` to leave this session**, then run `/plato <ticket-number>` to start this task over." (for `simple_feature`, say "start over" instead of "start this task over" — there's no per-task concept)
